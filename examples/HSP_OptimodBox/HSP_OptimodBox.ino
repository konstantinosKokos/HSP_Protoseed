// HSP_OptimodBox_v3_18_96k_bs4_AM_constants_TS3invert.ino
...
// FS2/LED2: OCD-style PRE-DRIVE toggle before the processor. LED2 = pre-drive ON, forced OFF when bypassed.
...
// NOTE: HaroldPCB v1.3.0 — 96 kHz / 4-sample block. Mono only. Right out is muted by the library.
...
static const float kPreDriveFadeAlpha = 0.0040f;  // OCD pre-drive in/out

// Master volume taper (0..1)^exp → natural to true zero
static const float kMasterPowerExp = 2.5f;

// Pre-drive (OCD-ish) voicing
static const float kOD_HPF_Hz = 100.0f;     // input rumble removal
static const float kOD_TiltHP_Hz = 720.0f;  // pre-emphasis for bite
static const float kOD_LPF_Hz = 6000.0f;    // tame fizz
static const float kOD_Gain = 5.0f;         // overall drive
static const float kOD_TiltAmt = 0.25f;     // 0..~0.4 additional upper-mid push
...
// Ratio = base + slope*Amount, then biased by TS1
...
static HaroldPCB H;         // v1.3.0 instance name/style
...
struct PreDrive {
  ...
  OnePoleLP lpf_out;
  float gain = kOD_Gain;
  float tilt_amt = kOD_TiltAmt;
  void setupModerate() {
    hpf_in.setFC(kOD_HPF_Hz);
    hp_tilt.setFC(kOD_TiltHP_Hz);
    lpf_out.setFC(kOD_LPF_Hz);
    gain = kOD_Gain;
    tilt_amt = kOD_TiltAmt;
  }
  inline float process(float x) {
    float t = x;
    t = hpf_in.process(t);
...
// HaroldPCB v1.3 maps FS (left/right) consistently. Keep indices explicit for clarity.
static constexpr int FS_BYP_IDX = 1;  // panel FS1 = BYPASS (right on your v1.2 wiring; v1.3 handles polarity)
static constexpr int FS_OCD_IDX = 0;  // panel FS2 = OCD / Pre-drive
...
static inline void ShadowWarmStates(float rawIn) {
  ...
  float x_pre = gPre.process(x0e);
  float x_in = (1.0f - gDriveWet) * x0e + gDriveWet * x_pre;
...
static void AudioCallback(float in, float &out) {
  ...
  gDriveWet += kPreDriveFadeAlpha * (gDriveTarget - gDriveWet);
...
static void QuiesceUnusedPins() {
  for (int i = 2; i < 8; ++i) { H.SetLED(i, /*OFF*/ true); }
}

void setup() {
  // Audio at 96 kHz / 4-sample block (HaroldPCB v1.3.0)
  H.Init(96000, 4);
...
void loop() {
  ...
  // Debounce both footswitches (library v1.3.0 also hardens edge logic; we keep local debounce for extra safety)
  Debounce &b1 = dbFS1;
  Debounce &b2 = dbFS2;
  b1.update(H.FootswitchIsPressed(FS_BYP_IDX), nowMs, DEB_MS);
  b2.update(H.FootswitchIsPressed(FS_OCD_IDX), nowMs, DEB_MS);

  // BYPASS logic (always available)
  if (b1.rose()) {
    gEffectOn = !gEffectOn;
    gActiveTarget = gEffectOn ? 1.0f : 0.0f;
    gDriveTarget = gEffectOn ? (gDriveOn ? 1.0f : 0.0f) : 0.0f;  // force pre-drive off in bypass
  }

  // OCD PRE-DRIVE (only when engaged)
  ...
  float f_raw = H.ReadPotSmoothed(RV1, 25.0f);
  float a_raw = H.ReadPotSmoothed(RV2, 25.0f);
  float t_raw = H.ReadPotSmoothed(RV3, 25.0f);
  float h_raw = H.ReadPotSmoothed(RV4, 25.0f);
  float m_raw = H.ReadPotSmoothed(RV5, 25.0f);
  float master_raw = H.ReadPotSmoothed(RV6, 25.0f);

  // Smooth master taper (0..1)^exp to true zero — **applies only to processed branch**
  float x = master_raw;
  if (x < 0.0f) x = 0.0f;
